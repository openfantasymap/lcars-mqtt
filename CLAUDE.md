...
**Per-station layouts:** `ConnectorService.setupStation()` resolves a station's layout from three sources, highest priority first: the GM **screen registry** `screens[id]`, the legacy `{room}/{station}/form` push, then the `/assets/stations.json` seed (keyed by station id, `default` fallback). Each layout is a station `form`: `{title, footer, layout?, structure: {sidebar[], content{tab: <container>}}, sources[]}`.

**Chrome layouts** (`InterfaceComponent`, built from lcars-ngx `lcars-elbow`/`lcars-bar`/`lcars-nav`): the optional top-level `layout` field selects the frame — `'sidebar'` (default: top-left elbow → top title bar, flush left nav rail, content) or `'frame'` (full left bracket: top elbow+bar, left rail, bottom elbow+bar/footer). Container widget `type`s the renderer supports: structural `row`/`column`/`panel`/`bar`/`text`; command `button`/`toggle`/`nav` (D-pad); display `gauge`/`compass`/`helm`/`slider`/`bargraph`/`indicator`/`readout`/`alert`/`power`/`warpcore` (live value from `{room}/io/{source}`, else static `value`); domain `issues`/`stats`. (`lcars-compass` currently doesn't size correctly — use the `nav` D-pad for directional controls.)
...
- **Zone change detection is required.** Angular 21 defaults to **zoneless**, but this app updates the view from rxjs/MQTT/HTTP subscriptions (not signals). `app.config.ts` calls `provideZoneChangeDetection({eventCoalescing:true})` and `angular.json` bundles the `zone.js` polyfill. Remove either and the UI renders once then **freezes** at initial values (blank chrome, empty screens) — with no error. Station ids are lowercased in `StationComponent` so `/lcars/:room/COMMS` resolves the lowercase `comms` screen/registry key.
